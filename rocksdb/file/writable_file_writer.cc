...
IOStatus WritableFileWriter::Append(const IOOptions& opts, const Slice& data,
                                    uint32_t crc32c_checksum, bool is_data_block) {
  if (seen_error()) {
  ...
  size_t left = data.size(), asize = 0;
  IOStatus s;
  pending_sync_ = true;

  // decide compress_mode_ by filename (*.sst)
  bool compress_mode_ = is_sst_file_ & is_data_block;
  if (compress_mode_)
    assert(use_direct_io());

  // printf("in WritableFileWriter::Append, fname %s data size %lu, data block? %d compress? %d\n", 
  //   file_name_.c_str(), left, is_data_block, compress_mode_);
  ...
  if (perform_data_verification_ && buffered_data_with_checksum_ &&
    ...
    if (use_direct_io() || (buf_.Capacity() >= left)) {
      while (left > 0) {
        if (compress_mode_) {
          // data blocks can not be separated in RCZNS
          size_t buffer_remaining = buf_.Capacity() - buf_.CurrentSize();
          if (buffer_remaining >= left) {
            size_t appended = buf_.Append(src, left);
            // TO FIX: currently data block size can not exceed 64KB
            if (appended >= (1 << 16))
              printf("metadata entry overflow! %lu\n", appended);
            left -= appended;
            src += appended;

            metadata_vec_.push_back((uint16_t)appended);
            prev_length_vec_.push_back((uint16_t)appended);
          } 
          // buffer_remaining < left
          if (left > 0) {
            // translate metadata_vec_ to metadata
            size_t num = metadata_vec_.size(), oob_size = 64; // TO FIX
            size_t metadata_size = Roundup(buf_.CurrentSize(), buf_.Alignment()) / buf_.Alignment() * oob_size;

            uint16_t* metadata = (uint16_t *)malloc(metadata_size);
            memset(metadata, 0, metadata_size);
            *metadata = (uint16_t)num;
            memcpy(metadata + 1, metadata_vec_.data(), num * sizeof(uint16_t));
            metadata_vec_.clear();

            s = Flush(io_options, (void *)metadata, num);
            if (!s.ok()) {
              break;
            }
            // TO FIX: When buffer cap is less than write I/O size, this may cause less filesize increasement.
            asize = buf_.Alignment() * metadata[num + 1] + metadata[num + 2];
            free(metadata);
          }
        } else {
          size_t appended = buf_.Append(src, left);
  ...
  if (s.ok()) {
    ...
    if (compress_mode_) {
      filesize_.store(cur_size + asize, std::memory_order_release);
    } else {
...
// enabled
IOStatus WritableFileWriter::Flush(const IOOptions& opts, void* metadata, 
                                   int metadata_num) {
  if (seen_error()) {
    return AssertFalseAndGetStatusForPrevError();
  }
    ...
    if (use_direct_io()) {
      if (pending_sync_) {
        if (perform_data_verification_ && buffered_data_with_checksum_) {
          ...
          s = WriteDirect(io_options, metadata, metadata_num);
        }
      }
    } else {
      if (perform_data_verification_ && buffered_data_with_checksum_) {
...
// offsets.
IOStatus WritableFileWriter::WriteDirect(IOOptions& opts, void* metadata, 
                                         int metadata_num) {
  if (seen_error()) {
    assert(false);
  ...
  while (left > 0) {
...

    {
      ...
      opts.metadata_num = metadata_num;
      ...
      // save the returned indexes
      uint16_t *tmp = nullptr;
      if (metadata) {
        tmp = (uint16_t *)metadata;
        asize = alignment * tmp[metadata_num + 1] + tmp[metadata_num + 2];
        file_advance -= (size - asize);

        for (int i = 0; i < metadata_num; i++)
          index_vec_.push_back(tmp[i + 1]);
      }

      if (ShouldNotifyListeners()) {
        assert(false);
        auto finish_ts = std::chrono::steady_clock::now();
        // TO FIX: not sure if 'size' should be changed accordingly
    ...
    src += size;
    // write_offset += size;
    write_offset += metadata? asize : size;
    uint64_t cur_size = flushed_size_.load(std::memory_order_acquire);
    flushed_size_.store(cur_size + size, std::memory_order_release);
    assert((next_write_offset_ % alignment) == 0);
#ifndef NDBG
    printf("after PositionedAppend, metadata? %d rev %lu, fs %lu\n", 
           metadata != nullptr, asize, filesize_.load(std::memory_order_acquire));
#endif
  }
  ...
  if (s.ok()) {
    ...
    // TODO: not sure the implication of this code segment
    // buf_.RefitTail(file_advance, leftover_tail);
    buf_.RefitTail(file_advance, 0);
    ...
    next_write_offset_ += file_advance + (!leftover_tail? 0 : alignment);
  } else {
    set_seen_error();
  }
  return s;

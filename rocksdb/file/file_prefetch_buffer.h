...
struct BufferInfo {
  ...
  // for RCZNS
  // hash_map is useful for locating the decompressed-read data in the prefetch buffer, 
  // maintaining the mapping: the original global offset ->   
  // <the new offset from the buffer start, the actual length after decompressed>.
  std::unordered_map<size_t, std::pair<size_t, uint16_t> > hash_map;
};
...
class FilePrefetchBuffer {
 public:
  ...
  size_t GetReadaheadSize() { return readahead_size_; }

  void SetReadaheadSize(const size_t size) { readahead_size_ = size; }
 ...
 private:
  ...
  void PrepareBufferForRead_RC(BufferInfo* buf, size_t alignment, uint64_t offset,
                            size_t roundup_len, bool refit_tail,
                            uint64_t& aligned_useful_len);
  ...
  void ReadAheadSizeTuning(BufferInfo* buf, bool read_curr_block,
                           ...
                           uint64_t& aligned_useful_len, bool rc = false);

...
class WritableFileWriter {
 ...
 public:
  WritableFileWriter(
    ...
    is_sst_file_ = false;
    size_t idx = _file_name.find_last_of('.');
    if (idx != std::string::npos) {
      if (_file_name.substr(idx + 1) == "sst")
        is_sst_file_ = true;
    }
  }
  ...
  IOStatus Flush(const IOOptions& opts, void* metadata = nullptr, 
                 int metadata_num = 0);
  ...
  IOStatus AddFileSize(uint64_t add) {
    uint64_t cur_size = filesize_.load(std::memory_order_acquire);
    filesize_.store(cur_size + add, std::memory_order_release);
    return IOStatus::OK();
  }
  ...
  size_t GetBufferSize() { return buf_.CurrentSize(); }

  size_t GetBufferAlignment() { return buf_.Alignment(); }
  ...
  // for RCZNS
  bool is_sst_file_;
  std::vector<uint16_t> metadata_vec_;
  std::vector<uint16_t> prev_length_vec_;
  std::vector<uint16_t> index_vec_;
 ...
 private:
  ...
  IOStatus WriteDirect(IOOptions& opts, void* metadata = nullptr, 
                       int metadata_num = 0);
  // `opts` should've been called with `FinalizeIOOptions()` before passing in

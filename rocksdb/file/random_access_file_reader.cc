...
IOStatus RandomAccessFileReader::Read(const IOOptions& opts, uint64_t offset,
...
#ifndef NDBG
  printf("in RandomAccessFileReader::Read, o %lu n %lu scratch? %d aligned_buf? %d result? %d\n", 
         offset, n, scratch != nullptr, aligned_buf != nullptr, result != nullptr);
#endif
...

  {
    ...
    if (use_direct_io() && is_aligned == false) {
      ...
      size_t aligned_offset, offset_advance, read_size;
      if (opts.decompressed_read) {
        // aligned is not neccessary in decompressed-read mode
        aligned_offset = static_cast<size_t>(offset);
        offset_advance = 0;
        read_size = static_cast<size_t>(n);
        // TO FIX: potential overflow (very annoyed)
        // if (opts.flexible_mode)
        size_t new_sz = std::max(static_cast<size_t>(opts.actual_size << 2),
                                 static_cast<size_t>(16384));
        buf.AllocateNewBuffer(std::max(static_cast<size_t>(read_size << 2), new_sz)); 
        // else
        //   buf.AllocateNewBuffer(Roundup(static_cast<size_t>(opts.actual_size), alignment)); 
      } else {
        aligned_offset =
            TruncateToPageBoundary(alignment, static_cast<size_t>(offset));
        offset_advance = static_cast<size_t>(offset) - aligned_offset;
        read_size = Roundup(static_cast<size_t>(offset + n), alignment) - aligned_offset;
        buf.AllocateNewBuffer(read_size);
      }
#ifndef NDBG
      printf("buf cursz %lu cap %lu\n", buf.CurrentSize(), buf.Capacity());
#endif
      size_t asize = 0;
      while (buf.CurrentSize() < read_size) {
        ...
        if (ShouldNotifyListeners()) {
          ...
          // TO FIX: not sure if allowed (rather than tmp.size()) is right?
          NotifyOnFileReadFinish(orig_offset, allowed, start_ts, finish_ts,
                                 io_s);
          if (!io_s.ok()) {
        ...
        asize = tmp.size();
        if (buf.CurrentSize() + asize > buf.Capacity())
          printf("buffer overflow, cur %lu asize %lu cap %lu\n", 
                  buf.CurrentSize(), asize, buf.Capacity());
      ...
      if (io_s.ok() && offset_advance < buf.CurrentSize()) {
        res_len = std::min(buf.CurrentSize() - offset_advance, opts.decompressed_read? asize : n);
#ifndef NDBG
        printf("res_len %lu\n", res_len);
#endif
        if (aligned_buf == nullptr) {
#ifndef NDBG
          printf("cur %lu cap %lu from %lu read into buf\n", buf.CurrentSize(), buf.Capacity(), offset_advance);
#endif
          buf.Read(scratch, offset_advance, res_len);
        } else {
#ifndef NDBG
          printf("cur %lu cap %lu from %lu then read\n", buf.CurrentSize(), buf.Capacity(), offset_advance);
#endif

...
void FilePrefetchBuffer::PrepareBufferForRead_RC(BufferInfo* buf, size_t alignment,
  ...
  // TO FIX: currently I see in rare cases we need to copy data, just assume all data
  // in buffer is not needed anymore.

  // Create a new buffer only if current capacity is not sufficient, and memcopy
  // bytes from old buffer if needed (i.e., if aligned_useful_len is greater
  // than 0).
  if (buf->buffer_.Capacity() < (roundup_len << 2)) { // TO FIX
    buf->buffer_.Alignment(alignment);
    uint64_t new_sz = std::max(static_cast<size_t>(roundup_len << 2),
                               static_cast<size_t>(16384));
    buf->buffer_.AllocateNewBuffer(
        new_sz, false /* copy_data_to_new_buffer */,
        chunk_offset_in_buffer, static_cast<size_t>(aligned_useful_len));
  } else if (aligned_useful_len > 0 && refit_tail) {
    // New buffer not needed. But memmove bytes from tail to the beginning since
    // aligned_useful_len is greater than 0.
...
Status FilePrefetchBuffer::Read(BufferInfo* buf, const IOOptions& opts,
  ...
  // result.size() here may represent the actual decompressed length of data block
  buf->buffer_.Size(static_cast<size_t>(aligned_useful_len) + result.size());
#ifndef NDBG
  printf("received, chunk_len %lu size %lu\n", aligned_useful_len, result.size());
#endif
  return s;
}
...
Status FilePrefetchBuffer::Prefetch(const IOOptions& opts,
  ...
  if (!enable_ || reader == nullptr) {
    printf("prefetch is not enabled or reader is nullptr\n");
    return Status::OK();
  }
...
#ifndef NDBG
  printf("in FilePrefetchBuffer::Prefetch, dr? %d o %lu n %lu\n", opts.decompressed_read, offset, n);
#endif
  if (opts.decompressed_read) {
    auto& hash_map = buf->hash_map;
    auto it = hash_map.find(offset);
    // if offset can be found in hash_map, this data block must be stored in the buffer
    if (it != hash_map.end()) {
      printf("got the data in the prefetch buffer, dr? %d\n", opts.decompressed_read);
      return Status::OK();
    }
  } else {
  ...
  ReadAheadSizeTuning(buf, /*read_curr_block=*/true,
                      ...
                      aligned_useful_len, opts.decompressed_read);
...
void FilePrefetchBuffer::ReadAheadSizeTuning(
    ...
    size_t& read_len, uint64_t& aligned_useful_len, bool rc) {
  ...
  if (!rc) {
    ...
    end_offset = start_offset + length + readahead_size;
  }
...
#ifndef NDBG
  printf("in FilePrefetchBuffer::ReadAheadSizeTuning, start %lu rulen %lu\n", start_offset, roundup_len);
#endif
  if (rc) {
    PrepareBufferForRead_RC(buf, alignment, start_offset, roundup_len,
                            refit_tail, aligned_useful_len);
  } else {
...
#ifndef NDBG
  printf("buf ofst %lu ini_end_ofst %lu rl %lu\n", start_offset, initial_end_offset, read_len);
#endif
  UpdateReadAheadTrimmedStat((initial_end_offset - initial_start_offset),
                             (end_offset - start_offset));
}
...
bool FilePrefetchBuffer::TryReadFromCacheUntracked(
...
#ifndef NDBG
  printf("in FilePrefetchBuffer::TryReadFromCacheUntracked, o %lu n %lu rasz %lu bcursz %lu\n", 
          offset, n, readahead_size_, buf->buffer_.CurrentSize());
#endif
  ...
  size_t asize = n;
  // Remap offset and n to real sizes
  auto& hash_map = buf->hash_map;
  auto it = hash_map.find(offset);
  uint64_t offset_bp = offset;
  bool find_it = false, flag;
  if (it != hash_map.end()) {
#ifndef NDBG
    printf("CP1 renew requested range [%lu, %lu] -> [%lu, %d]\n", offset, n, it->second.first, it->second.second);
#endif
    // The remapped size could possibly be even smaller, because data block could be padded to page boundary when compressed.
    // assert(n <= it->second.second);
    offset = buf->offset_ + it->second.first;
    asize = it->second.second;
    find_it = true;
  } 

  // For RCZNS, if offset can not be found in hash_map, data MUST not be stored in buffer and needs fetched.
  flag = opts.decompressed_read & !find_it;
  if (explicit_prefetch_submitted_ ||
      (buf->async_read_in_progress_ ||
       offset + asize > buf->offset_ + buf->CurrentSize() || flag)) {
    // In case readahead_size is trimmed (=0), we still want to poll the data
    // submitted with explicit_prefetch_submitted_=true.
#ifndef NDBG
    printf("here, ras %lu fc? %d iar? %d dr? %d\n", readahead_size_, for_compaction, implicit_auto_readahead_, opts.decompressed_read);
#endif
    if (readahead_size_ > 0 || explicit_prefetch_submitted_) {
      ...
      if (for_compaction) {
        s = Prefetch(opts, reader, offset, std::max(asize, readahead_size_));
      } else {
        if (implicit_auto_readahead_) {
#ifndef NDBG
          printf("#reads %lu sequential? %d ofst %lu cur [%lu %lu] prev [%lu %lu]\n", num_file_reads_, IsBlockSequential(offset), 
                  offset, offset_bp, asize, prev_offset_, prev_len_);
#endif
          if (!IsEligibleForPrefetch(offset_bp, asize)) {
    ...
    if (opts.alens_addr) {
      ...
      hash_map.clear();
      uint64_t offset_ac = offset, offset_ad = offset - buf->offset_;
#ifndef NDBG
      uint64_t min_offset_ac = offset_ac, min_offset_ad = offset_ad;
#endif
      for (int i = 1; i <= *tmp; i++) {
        // printf("-- %lu -> <%lu %d>\n", offset_ac, offset_ad, tmp[(i << 1) - 1]);
        hash_map.insert(std::make_pair(offset_ac, std::make_pair(offset_ad, tmp[(i << 1) - 1])));
        offset_ac += tmp[i << 1];
        offset_ad += tmp[(i << 1) - 1];  
      }
#ifndef NDBG
      printf("renew hashmap [%lu, %lu] -> [%lu, %lu], cursz %lu\n", min_offset_ac, offset_ac, min_offset_ad, offset_ad,
              hash_map.size());
#endif
      free(tmp);
    }
  ...
  UpdateReadPattern(offset_bp, n, /*decrease_readaheadsize=*/false);

  if (!find_it) {
    it = hash_map.find(offset);
    if (it != hash_map.end()) {
#ifndef NDBG
      printf("CP2 renew requested range [%lu, %lu] -> [%lu, %d]\n", offset, n, it->second.first, it->second.second);
#endif
      offset = buf->offset_ + it->second.first;
      asize = it->second.second;
    } 
  ...
  *result = Slice(buf->buffer_.BufferStart() + offset_in_buffer, asize);
#ifndef NDBG
  printf("finish try read from cache, offset %lu asize %lu oib %lu\n", offset, asize, offset_in_buffer);
#endif
  if (prefetched) {

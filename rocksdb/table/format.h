...
class BlockHandle {
 public:
  ...
  BlockHandle(uint64_t offset, uint64_t size, uint64_t prev_size);
  ...
  // The previous size of the stored block (useful in RCZNS)
  uint64_t prev_size() const { return prev_size_; }
  void set_prev_size(uint64_t _prev_size) { prev_size_ = _prev_size; }
...
inline BlockHandle::BlockHandle(uint64_t _offset, uint64_t _size, uint64_t _prev_size)
    : offset_(_offset), size_(_size), prev_size_(_prev_size) {}

...
class BlockFetcher {
 public:
  BlockFetcher(RandomAccessFileReader* file,
      ...
      : file_(file),
        ...
        decompressed_read_(decompressed_read),
        flexible_mode_(flexible_mode) {
 ...
 private:
  ...
  void CopyBufferToHeapBuf(size_t asize = 0);
  ...
  void ProcessTrailerIfPresent_CZNS(size_t asize = 0);
};
}  // namespace ROCKSDB_NAMESPACE

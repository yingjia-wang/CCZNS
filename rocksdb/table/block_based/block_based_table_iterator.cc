...
void BlockBasedTableIterator::InitDataBlock() {
  ...
  size_t value = data_block_handle.size(), shift = 0;
  ...
  compressed_size |= (value & 0x7) << shift;
  data_block_handle.set_prev_size(value >> 4);
  data_block_handle.set_size(compressed_size);
#ifndef NDBG
  printf("in BlockBasedTableIterator::InitDataBlock, user-requested ras %lu, o %lu s %lu\n", 
          read_options_.readahead_size, data_block_handle.offset(), data_block_handle.size());
#endif
  if (!block_iter_points_to_real_block_ ||
    ...
    if (is_in_cache) {
      ...
      // In RCZNS, readahead size cannot exceed the boundary between data blocks and index block.
      auto prefetch_buffer = block_prefetcher_.prefetch_buffer(); 
      if (prefetch_buffer) {
        uint64_t prefetch_size = 262144;
        uint64_t max_prefetch_size = rep->footer.index_handle().offset() - data_block_handle.offset();
        // TO FIX: The upper bound of next is limited by the maximum bytes ioctl can transfer
        prefetch_size = std::min(prefetch_size, max_prefetch_size);   
  #ifndef NDBG
        printf("adjust ras from %lu to %lu, max %lu\n", prefetch_buffer->GetReadaheadSize(), prefetch_size, max_prefetch_size);
  #endif
        prefetch_buffer->SetReadaheadSize(prefetch_size);
      }

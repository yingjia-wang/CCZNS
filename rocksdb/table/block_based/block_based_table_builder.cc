...
void BlockBasedTableBuilder::Add(const Slice& key, const Slice& value) {
  ...
  if (IsValueType(value_type)) {
    ...
    if (should_flush) {
      ...
      // Postpone but save until all data blocks are flushed
      if (ok() && r->state == Rep::State::kUnbuffered) {
        //std::hash<std::string> str_hash;
        if (r->IsParallelCompressionEnabled()) {
          r->pc_rep->curr_block_keys->Clear();
        } else {
          // r->last_key is shortened and used later in this func
          // printf("ADD STAGE %luth last_key %lu ", r->keys.size(), str_hash(r->last_key));
          r->index_builder->FindShortestInternalKey(&r->last_key, &key);
          // printf("-> %lu key %lu\n", str_hash(r->last_key), str_hash(key.ToString()));
        }

        // save the key and make index entry later
        r->last_keys.emplace_back(r->last_key);
        r->keys.emplace_back(key.data(), key.size());
      }
    }
...
void BlockBasedTableBuilder::WriteBlock(const Slice& uncompressed_block_data,
  ...
  if (is_data_block) {
    r->props.data_size = r->get_offset(); // TO FIX
...
void BlockBasedTableBuilder::WriteMaybeCompressedBlock(
  ...
  // concatenate data block with its trailer and append together
  std::string block_contents_wt;
  block_contents_wt.reserve(block_contents.size() + trailer.size());
  block_contents_wt.append(block_contents.data(), block_contents.size());
  block_contents_wt.append(trailer.data(), trailer.size());

  {
    io_s = r->file->Append(io_options, 
                           Slice(block_contents_wt.data(), block_contents_wt.size()), 
                           0, is_data_block);
    if (!io_s.ok()) {
      r->SetIOStatus(io_s);
      return;
...
#ifndef NDBG
  printf("in BlockBasedTableBuilder::WriteIndexBlock\n");
#endif
...
#ifndef NDBG
  printf("index o %lu s %lu\n", index_block_handle.offset(), index_block_handle.size());
  printf("meta o %lu s %lu\n", metaindex_block_handle.offset(), metaindex_block_handle.size());
  printf("footer s %lu version %u\n", footer.GetSlice().size(), r->table_options.format_version);
#endif
  if (ios.ok()) {
...
Status BlockBasedTableBuilder::Finish() {
  ...
  // flush indexes from the last data block first
  auto& vec = r->file->metadata_vec_;
  size_t num = vec.size(), oob_size = 64; // TO FIX
  size_t metadata_size = Roundup(r->file->GetBufferSize(), r->file->GetBufferAlignment()) / 
                                 r->file->GetBufferAlignment() * oob_size;

  // TO FIX: currently only supports data block as large as 65535 bytes
  uint16_t* metadata = (uint16_t *)malloc(metadata_size);
  memset(metadata, 0, metadata_size);
  *metadata = (uint16_t)num;
  memcpy(metadata + 1, vec.data(), num * sizeof(uint16_t));
  vec.clear();

  IOOptions opts;
  IOStatus s = WritableFileWriter::PrepareIOOptions(r->write_options, opts);
  r->file->Flush(opts, (void *)metadata, num);

  // manually add the file size
  uint64_t asize = r->file->GetBufferAlignment() * metadata[num + 1] + metadata[num + 2];
  r->file->AddFileSize(asize);
  free(metadata);

  size_t sum, value, j, output, shift, tmp;
  if (r->state == Rep::State::kBuffered) {
    assert(false);
    EnterUnbuffered();
  }
  if (r->IsParallelCompressionEnabled()) {
    ...
    auto& ivec = r->file->index_vec_;
    auto& pvec = r->file->prev_length_vec_;
    assert(ivec.size() == pvec.size());
    assert(ivec.size() == r->props.num_data_blocks);
    assert(r->keys.size() == ivec.size() - 1);

    // construct index entries of the whole sst
    sum = 0;
    // std::hash<std::string> str_hash;
    // printf("CHECK STAGE\n");
    output = shift = 0;
    for (size_t i = 0; i < r->keys.size(); i++) {
      ...
      tmp = ivec[i];
      while (ivec[i] >= 0x8) {
        // printf("prev %d ivec[i] %d sft %lu\n", pvec[i], ivec[i], shift);
        output |= ((ivec[i] & 0x7) | 0x8) << shift;
        ivec[i] >>= 3;
        shift += 4;
      }
      output |= (ivec[i] & 0x7) << shift;
      value = ((size_t)(pvec[i]) << (shift + 4)) + output;
      output = shift = 0;
      // printf("%luth %lu -> %lu\n", i, str_hash(r->last_keys[i]), str_hash(ss.ToString()));
      r->index_builder->AddIndexEntry(
          &r->last_keys[i], &ss, (BlockHandle){sum, value});
      sum += tmp;
    }
    j = ivec.size() - 1;
    if (ok() && !empty_data_block) {
      // from the last data block (may be incomplete)
      tmp = ivec[j];
      while (ivec[j] >= 0x8) {
        // printf("prev %d ivec[j] %d sft %lu\n", pvec[j], ivec[j], shift);
        output |= ((ivec[j] & 0x7) | 0x8) << shift;
        ivec[j] >>= 3;
        shift += 4;
      }
      output |= (ivec[j] & 0x7) << shift;
      value = ((size_t)(pvec[j]) << (shift + 4)) + output;
      r->index_builder->AddIndexEntry(
          &r->last_key, nullptr /* no next data block */, (BlockHandle){sum, value});
      sum += tmp;
    }
#ifndef NDBG
    printf("sst size %lu #data blocks %lu\n", sum, r->props.num_data_blocks);
#endif
    assert(sum % r->file->GetBufferAlignment() == 0);
  }
  r->set_offset(sum);

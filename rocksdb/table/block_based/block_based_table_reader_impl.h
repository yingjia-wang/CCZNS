...
TBlockIter* BlockBasedTable::NewDataBlockIterator(
...
#ifndef NDBG
  printf("in BlockBasedTable::NewDataBlockIterator, o %lu s %lu(%lu) compaction? %d prefetch_buffer? %d\n", 
          handle.offset(), handle.size(), handle.prev_size(), for_compaction, prefetch_buffer != nullptr);
#endif
  ...
  if (rep_->uncompression_dict_reader && block_type == BlockType::kData) {
    ...
    s = RetrieveBlock(
        ...
        /* use_cache */ true, async_read, use_block_cache_for_lookup);
#ifndef NDBG
        printf("haha retrieve block %lu bytes back\n", block.GetValue()->size());
#endif
  }
...
TBlockIter* BlockBasedTable::NewDataBlockIterator(const ReadOptions& ro,
  ...
  PERF_TIMER_GUARD(new_table_block_iter_nanos);
#ifndef NDBG
  printf("in another BlockBasedTable::NewDataBlockIterator\n");
#endif
  TBlockIter* iter = input_iter != nullptr ? input_iter : new TBlockIter;
  if (!s.ok()) {
    iter->Invalidate(s);

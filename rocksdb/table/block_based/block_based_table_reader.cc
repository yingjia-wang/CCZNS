...
Status ReadAndParseBlockFromFile(
    ...
    bool decompressed_read, bool flexible_mode) {
  assert(result);
  ...
  BlockFetcher block_fetcher(
      ...
      decompressed_read, flexible_mode);
  Status s;
...
#ifndef NDBG
  printf("in BlockBasedTable::ReadRangeDelBlock\n");
#endif
  Status s;
...
Status BlockBasedTable::ReadMetaIndexBlock(
  ...
  Status s = ReadAndParseBlockFromFile(
      ...
      false /* async_read */, false /* decompressed_read */, false /* flexible_mode */);
...
WithBlocklikeCheck<Status, TBlocklike> BlockBasedTable::GetDataBlockFromCache(
  ...
  assert(out_parsed_block->IsEmpty());
#ifndef NDBG
  printf("Fail to find block in BlockBasedTable::GetDataBlockFromCache\n");
#endif
...
BlockBasedTable::MaybeReadBlockAndLoadToCache(
  ...
  if (block_cache) {
        ...
        if (out_parsed_block->GetValue() ||
          ...
          if (prefetch_buffer) {
            ...
            prefetch_buffer->UpdateReadPattern(
                handle.offset(), handle.size(), // BlockSizeWithTrailer(handle),
    ...
    if (out_parsed_block->GetValue() == nullptr &&
        ...
        ro.fill_cache) {
#ifndef NDBG
      printf("Fail to find from the cache, proceed\n");
#endif
      ...
      if (!contents) {
        ...
        BlockFetcher block_fetcher(
            ...
            /*allocator=*/nullptr, false /* for_compaction */,
            true /* decompressed_read */, false /* flexible_mode */);
...
WithBlocklikeCheck<Status, TBlocklike> BlockBasedTable::RetrieveBlock(
  ...
  assert(out_parsed_block->IsEmpty());
#ifndef NDBG
  printf("in BlockBasedTable::RetrieveBlock, use_cache? %d\n", use_cache);
#endif
  Status s;
  if (use_cache) {
...

  {
    ...
    // In RCZNS, data blocks and non data blocks should be considered in different modes
    uint64_t index_offset = rep_->footer.index_handle().offset();
    bool decompressed_read = handle.offset() < index_offset;
    if (decompressed_read && handle.offset() + handle.size() > index_offset) assert(false);
#ifndef NDBG
    printf("begin ReadAndParseBlockFromFile(%s), o %lu s %lu io %lu cache? %d dr? %d\n", rep_->file->file_name().c_str(), 
      handle.offset(), handle.size(), index_offset, use_cache, decompressed_read);
#endif
    ...
    s = ReadAndParseBlockFromFile(
        ...
        decompressed_read, true /* flexible_mode */);
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  if (may_match) {
    ...
    for (iiter->Seek(key); iiter->Valid() && !done; iiter->Next()) {
      ...
      size_t value = v.handle.size(), shift = 0;
      size_t compressed_size = 0;
      while ((value & 0x8) != 0) {
        compressed_size |= (value & 0x7) << shift;
        value >>= 4;
        shift += 3;
      }
      compressed_size |= (value & 0x7) << shift;
      v.handle.set_prev_size(value >> 4);
      v.handle.set_size(compressed_size);
...
#ifndef NDBG
  printf("in BlockBasedTable::Prefetch\n");
#endif

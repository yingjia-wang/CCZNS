...
IOStatus Zone::Append(char *data, uint32_t size, void *metadata, int metadata_num, 
                       size_t* asize) {
  ...
  // Compressed / Non-compressed data both use ioctl rather than pwrite currently.
  uint32_t upper = 1048576; 
  if (size > upper) { 
    printf("error! unexpected size %u bytes!\n", size);
    len = upper;
  }

  uint32_t block_sz = zbd_->GetBlockSize(), oob_sz = zbd_->GetOOBSize();
  uint32_t metadata_len = 0;

  bool multi_cw_enabled = true;
  if (multi_cw_enabled || (!multi_cw_enabled && metadata == nullptr)) {
    if (metadata)
      metadata_len = size / block_sz * oob_sz;
    ...
    struct nvme_passthru_cmd cmd = {
      ...
      .data_len = len,
      .cdw10 = (__u32)(wp & 0xffffffff),
      .cdw11 = (__u32)(wp >> 32),
      .cdw12 = (__u32)(len >> 12) - 1,
    };
    if (metadata) cmd.cdw12 |= (1 << 16);


    // printf("write! start %lu wp %lu mlen %u len %u w2 %x w3 %x w10 %x w11 %x w12 %x\n", 
    //         start_ / (256*1024*1024), wp, metadata_len, len, cmd.cdw2, cmd.cdw3, cmd.cdw10, cmd.cdw11, cmd.cdw12);
    // ret = pwrite(fd, ptr, len, wp_);
    ...
    // zbd_->avg_write_io_time = (zbd_->avg_write_io_time * zbd_->write_io_count + d.count()) / (zbd_->write_io_count + 1);
    // zbd_->write_io_count ++;
    ...
    uint16_t *tmp = metadata? (uint16_t *)metadata : nullptr;
    if (metadata) {
      *asize = ret = block_sz * tmp[metadata_num + 1] + tmp[metadata_num + 2];
    } else {
      ret = len;
    }
    ...
    // if (zbd_->write_io_count % 1000000000)
    //   printf("write epoch %lu: avg %lf ms\n", zbd_->write_io_count, zbd_->avg_write_io_time);
  } else {
    // !multi_cw_enabled && metadata
    uint16_t* tmp = (uint16_t*)metadata;
    uint16_t* new_metadata = nullptr;
    uint32_t num_partitions = *tmp, chunk_sz;
    size_t total_bytes = 0;

    for (uint32_t i = 1; i <= num_partitions; i++) {
      chunk_sz = tmp[i];
      len = (chunk_sz % block_sz == 0? chunk_sz : (chunk_sz + block_sz - chunk_sz % block_sz));
      metadata_len = len / block_sz * oob_sz;
      new_metadata = (uint16_t*)malloc(metadata_len);
      new_metadata[0] = 1;
      new_metadata[1] = chunk_sz;

      struct nvme_passthru_cmd cmd = {
        .opcode = 1, // write
        .nsid = 1, // TO FIX
        .metadata = (__u64)new_metadata,
        .addr = (__u64)ptr,
        .metadata_len = metadata_len,
        .data_len = len,
        .cdw10 = (__u32)(wp & 0xffffffff),
      ...
      ret = block_sz * new_metadata[2] + new_metadata[3];
      tmp[i] = ret;
      total_bytes += ret;
      free(new_metadata);

      // printf("chunk #%u cksz %u len %u mlen %u ret %d tt %lu\n", 
      //        i, chunk_sz, len, metadata_len, ret, total_bytes);
    ...
    *asize = total_bytes;
    tmp[num_partitions + 1] = total_bytes / block_sz;
    tmp[num_partitions + 2] = total_bytes % block_sz;
  }
...
ZonedBlockDevice::ZonedBlockDevice(std::string bdevname,
  ...
  avg_read_io_time = read_io_count = 0;
  avg_write_io_time = write_io_count = 0; 
}
...
IOStatus ZonedBlockDevice::Open(bool readonly, bool exclusive) {
  ...
  if (info.max_nr_active_zones == 0)
    max_nr_active_io_zones_ = 14; //info.nr_zones; // TO FIX
  else
    max_nr_active_io_zones_ = info.max_nr_active_zones - reserved_zones;

  if (info.max_nr_open_zones == 0)
    max_nr_open_io_zones_ = 14; //info.nr_zones; // TO FIX
  else
...
uint32_t ZonedBlockDevice::GetOOBSize() { return oob_sz_; }

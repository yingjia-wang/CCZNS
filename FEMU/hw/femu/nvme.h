...
typedef struct NvmeRequest {
    ...
    bool                    is_compress;
    bool                    is_decompress;
    void*                   data; // compressed or decompressed
    void*                   metadata;
    uint64_t                data_pages;
    uint64_t                byte_start;
...
typedef struct FemuCtrl {
    ...
    struct rte_ring **to_master;
    pqueue_t        **pq;
    bool            *should_isr;
    bool            poller_on;

    int64_t         nr_avg_read_late_ios;
    int64_t         nr_avg_write_late_ios;
    int64_t         nr_tt_ios;
    int64_t         nr_tt_read_ios;
    int64_t         nr_tt_write_ios;
    int64_t         nr_tt_late_ios;
    int64_t         nr_tt_read_late_ios;
    int64_t         nr_tt_write_late_ios;
    bool            print_log;

    uint8_t         multipoller_enabled;
    uint8_t         space_amp;
    uint8_t         page_padding;
    uint32_t        num_poller;
    uint32_t        num_master;
    uint32_t        next_master;
    uint32_t        num_worker;

    void            **mst_data;
    void            **mst_metadata;
    uint32_t        *mst_remaining;
    uint32_t        *mst_finished;
    uint32_t        *mst_total_bytes;
    uint32_t        *mst_start_byte;
    pthread_spinlock_t *mst_mutex;
    pthread_spinlock_t *mst_mutex2;

    uint32_t        min_compressed_time;
    uint32_t        min_decompressed_time;
    uint32_t        fixed_compression_ratio;
...
#define femu_debug_time(fmt, ...) \
    do { printf("[FEMU](%lu) Dbg: " fmt, qemu_clock_get_ns(QEMU_CLOCK_REALTIME), ## __VA_ARGS__); } while (0)
#else
#define femu_debug(fmt, ...) \
    do { } while (0)
#define femu_debug_time(fmt, ...) \
...
#define femu_err_time(fmt, ...) \
    do { printf("[FEMU](%lu) Err: " fmt, qemu_clock_get_ns(QEMU_CLOCK_REALTIME), ## __VA_ARGS__); } while (0)

#define femu_log_time(fmt, ...) \
    do { printf("[FEMU](%lu) Log: " fmt, qemu_clock_get_ns(QEMU_CLOCK_REALTIME), ## __VA_ARGS__); } while (0)

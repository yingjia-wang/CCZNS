...
static void femu_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    int64_t bs_size, allocated_size;
    ...
    allocated_size = bs_size * n->space_amp;
    init_dram_backend(&n->mbe, allocated_size);
...
static void nvme_destroy_poller(FemuCtrl *n)
{
    ...
    for (int i = 1; i <= n->num_master; i++) {
        qemu_thread_join(&n->master[i]);
    }

    for (int i = 1; i <= n->num_worker; i++) {
        qemu_thread_join(&n->worker[i]);
    }
    ...
    for (int i = 1; i <= n->num_master; i++) {
        pthread_spin_destroy(&n->mst_mutex[i]);
        pthread_spin_destroy(&n->mst_mutex2[i]);
    }
    free(n->mst_data);
    free(n->mst_metadata);
    free(n->mst_remaining);
    free(n->mst_finished);
    free(n->mst_total_bytes);
    free(n->mst_start_byte);

    for (int i = 1; i <= n->num_master; i++) {
        femu_ring_free(n->to_master[i]);
    }
...
static Property femu_props[] = {
    ...
    DEFINE_PROP_UINT8("space_amp", FemuCtrl, space_amp, 4),
    DEFINE_PROP_UINT8("page_padding", FemuCtrl, page_padding, 1),
    DEFINE_PROP_UINT32("num_master", FemuCtrl, num_master, 8),
    DEFINE_PROP_UINT32("num_worker", FemuCtrl, num_worker, 16),
    DEFINE_PROP_UINT32("min_compressed_time", FemuCtrl, min_compressed_time, 4000),
    DEFINE_PROP_UINT32("min_decompressed_time", FemuCtrl, min_decompressed_time, 3000),
    DEFINE_PROP_UINT32("fixed_compression_ratio", FemuCtrl, fixed_compression_ratio, 0),

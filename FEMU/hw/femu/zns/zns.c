...
static void zns_init_zone_identify(FemuCtrl *n, NvmeNamespace *ns, int lba_index)
{
    ...
    // allocate OOB
    n->oob_size = 64;
    for (int i = 0; i <= ns->id_ns.nlbaf; i++)
        ns->id_ns.lbaf[i].ms = n->oob_size; 
                                    ...
                                    uint32_t nlb)
{
    ...
    if(end > zone->w_ptr) {
        femu_debug("read over wp: slba %lu nlb %u end %lu wp %lu\n", slba, nlb, end, zone->w_ptr);
    }
                                     ...
                                     bool failed)
{
    ...
    if (req->is_compress) // if data has been compressed, we should add actual data length here
        zone->d.wp += req->data_pages;
    else
        zone->d.wp += nlb; 
                               ...
                               NvmeZoneState state, NvmeRequest *req)
{
    ...
    zonessd_reset(ns->ctrl, zone->pos, req);
                         ...
                         NvmeRequest *req)
{
    NvmeRwCmd *rw = (NvmeRwCmd *)&req->cmd;
    uint64_t slba = le64_to_cpu(rw->slba);
    uint32_t nlb = req->is_decompress? req->data_pages : ((uint32_t)le16_to_cpu(rw->nlb) + 1);
    uint64_t data_size = zns_l2b(ns, (uint32_t)le16_to_cpu(rw->nlb) + 1);
    uint16_t status;
    ...
    if (status) {
        femu_err("zns check read failed\n");
        goto err;
    }

    if (req->is_decompress) {
        // transfer modified metadata back to the host 
        backend_rw_data(req->metadata, &req->qsg, false);
        free(req->metadata);
    ...
    if (NVME_ERR_REC_DULBE(n->features.err_rec)) {
        ...
        if (status) {
            femu_err("zns check dulbe failed\n");
            goto err;
        }
    }

    // Non-compressed data is stored in their original positions, while
    // the locations of compressed data are specified by the offset_map.
    uint32_t zone_idx = zns_zone_idx(ns, slba);

    uint64_t data_offset = 0;
    femu_debug_time("read before DMA\n");
    if (req->is_decompress) {
        data_offset = zns_l2b(ns, n->zone_size * (n->num_zones + zone_idx * (n->space_amp - 1))) + req->byte_start;
        if (req->byte_start + req->byte_length > NVME_DEFAULT_ZONE_SIZE * (n->space_amp - 1))
          femu_err_time("preserve space error, start %lu length %lu\n", req->byte_start, req->byte_length);
        if (req->byte_length > data_size) // TO FIX
          femu_err_time("data lost error, length %lu vs %lu\n", req->byte_length, data_size);
        backend_rw_data_with_upper(n->mbe->logical_space + data_offset, &req->qsg, req->is_write, req->byte_length);
    } else {
        data_offset = zns_l2b(ns, slba);
        backend_rw(n->mbe, &req->qsg, &data_offset, req->is_write);
    }
    femu_debug_time("read after DMA\n");
    qemu_sglist_destroy(&req->qsg);

    return NVME_SUCCESS;

err:
    femu_err("*********ZONE READ FAILED*********\n");
    return status | NVME_DNR;
}
                          ...
                          NvmeRequest *req)
{
    ...
    // the locations of compressed data are specified by the offset_map.
    uint64_t data_offset = 0;
    if (req->is_compress) { // compressed write
        res->slba = zns_advance_zone_wp(ns, zone, req->data_pages);

        // transfer metadata back to the host  
        backend_rw_data(req->metadata, &req->qsg, false);
        free(req->metadata);
        ...
        res->slba = zns_advance_zone_wp(ns, zone, nlb);
        // struct zonessd_params *sp = &n->znsssd->sp;
        // Zone *z = &(n->znsssd->zone_array[zns_zone_idx(ns, slba)]);
        // z->byte_wp += nlb * sp->secsz * sp->secs_per_pg;
...
{
    uint16_t res;
    femu_debug("op %u, cid %u nsid %u res2 %lu cdw10 %u cdw11 %u cdw12 %u cdw13 %u cdw14 %u cdw15 %u\n",
        cmd->opcode, cmd->cid, cmd->nsid, cmd->res2, cmd->cdw10, cmd->cdw11, cmd->cdw12, cmd->cdw13, cmd->cdw14, cmd->cdw15);
    
    switch (cmd->opcode) {
    case NVME_CMD_READ:
        res = zns_read(n, ns, cmd, req);
        zonessd_read(n->znsssd, req);
        return res;
    case NVME_CMD_WRITE:
        res = zns_write(n, ns, cmd, req);
        zonessd_write(n->znsssd, req);
        return res;
    ...
    case NVME_CMD_ZONE_APPEND:
        zonessd_append(n->znsssd, req);
        break; // return zns_zone_append(n, req);
    }
...
static void zns_exit(FemuCtrl *n)
{
    ...
    pthread_spin_destroy(&n->znsssd->lock);
}

...
int backend_rw(SsdDramBackend *b, QEMUSGList *qsg, uint64_t *lbal, bool is_write)
{
    ...
    void *mb = b->logical_space + lbal[0];
    uint64_t mb_oft = 0;
...
void backend_rw_data(void *mb, QEMUSGList *qsg, bool is_write)
{
    int sg_cur_index = 0;
    dma_addr_t sg_cur_byte = 0;
    dma_addr_t cur_addr, cur_len;
    ...
    while (sg_cur_index < qsg->nsg) {
        ...
        if (dma_memory_rw(qsg->as, cur_addr, mb + mb_oft, cur_len, dir, MEMTXATTRS_UNSPECIFIED)) {
            femu_err("dma_rw_metadata error");
        }
        ...
        // assume only ZNS mode can call this function
        mb_oft += cur_len;
    }
}

void backend_rw_data_with_upper(void *mb, QEMUSGList *qsg, bool is_write, uint64_t upper)
{
    int sg_cur_index = 0;
    dma_addr_t sg_cur_byte = 0;
    dma_addr_t cur_addr, cur_len;
